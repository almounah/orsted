...
func (c *Client) CreateShell() (*Shell, error) {
	...
	request, err := mspsrp.EnvelopeToString(mspsrp.CreateShellRequest(creationXML, messageId, sessionID))
	if err != nil {
		return nil, fmt.Errorf("failed to create CreateShell XML: %w", err)
	}
	...
	response, err := c.sendRequest([]byte(request))
	if err != nil {
		return nil, err
	}
	...
	shellID, err := mspsrp.ParseShellID([]byte(response))
	if err != nil {
		return nil, err
	}
	for i := 0; i < 1; i++ {
		...
		debugger.Println("Draining Request ", i)
		debugger.Println("----------------------------")
		debugger.Println(request)
		debugger.Println("----------------------------")
		...
		debugger.Println("Draining Response ", i)
		debugger.Println("----------------------------")
		debugger.Println(response)
		debugger.Println("----------------------------")
		debugger.Println("----------------------------")
...
func (c *Client) NewShell(id, sessionId, runspaceId, pipelineId string) *Shell {
	return &Shell{Client: c, Id: id, SessionID: sessionId, PipelineID: pipelineId, RunspaceID: runspaceId}
}

// sendRequest exec the custom http func from the client
func (c *Client) sendRequest(request []byte) (string, error) {
