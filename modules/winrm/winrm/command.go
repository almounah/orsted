...
func (c *Command) Close() error {
	...
	_, err := c.client.sendRequest([]byte(request))
	return err
}
...
func (c *Command) slurpAllOutput() (bool, error) {
	...
	request, err := mspsrp.EnvelopeToString(mspsrp.CreateReceiveRequest(c.id, c.shell.Id, messageId, c.shell.SessionID))
	if err != nil {
		return false, fmt.Errorf("failed to create Receive XML: %w", err)
	}
	...
	response, err := c.client.sendRequest([]byte(request))
