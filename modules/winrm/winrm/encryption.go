...
func (e *Encryption) Post(client *Client, message []byte) (string, error) {
	...
	if err = e.PrepareRequest(client, client.url); err == nil {
		return e.PrepareEncryptedRequest(client, client.url, []byte(message))
	} else {
		return e.ntlm.Post(client, []byte(message))
	}
}
