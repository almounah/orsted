...
func SetUserInfo(username, hash string) func(*Client) error {
	return func(c *Client) error {
		c.username = username
		//c.password = password
		c.hash = hash
...
func (c *Client) processChallengeMessage(input []byte, bindings *ChannelBindings) ([]byte, error) {
	splitted := strings.Split(c.hash, ":")
	if len(splitted) != 2 {
		return nil, fmt.Errorf("Error in hash format, should be LM:NT")
	}
	lm := splitted[0]
	nt := splitted[1]
	debugger.Println("Process Challenge Messge", lm, ":", nt)
	cm := &challengeMessage{}
	...
	if err != nil {
		debugger.Println("Generate CHallenge Error", err)
		return nil, err
	}

	lmChallengeResponse, err := lmChallengeResponse(c.negotiatedFlags, c.compatibilityLevel, clientChallenge, c.username, c.hash, c.domain, cm)
	if err != nil {
		debugger.Println("Lm Challenge Response Error", err)
		return nil, err
	}

	targetInfo, err := cm.TargetInfo.Clone()
	if err != nil {
		debugger.Println("Target Clone Error", err)
		return nil, err
	}

	ntChallengeResponse, keyExchangeKey, err := ntChallengeResponse(c.negotiatedFlags, c.compatibilityLevel, clientChallenge, c.username, c.hash, c.domain, cm, lmChallengeResponse, *targetInfo, bindings)
	if err != nil {
		debugger.Println("Nt Challenge Resonse Error", err)
		return nil, err
	}
