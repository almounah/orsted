...
func LoadGoodClr(runtime string, NetBytes []byte) (*ICORRuntimeHost, []uint16, error) {
	...
	debugger.Println(fmt.Sprintf("[+] GetBindingIdentityFromStream: %s", syscall.UTF16ToString(identityBuffer)))
    ...
    debugger.Println("[?] Printing TargetAssembly otherwise it dies because of Go GC", tgtAssembly)
	...
	debugger.Println("[+] Set Custom Host Successfully")
    
	myCustomHost.Start()
	debugger.Println("[+] Started CLR Succesfully")
...
func PatchSysExit(appDomain *AppDomain) {
    ...
    if err != nil {
        debugger.Println("Error Returning ", err)
    }
    ...
    debugger.Println(fmt.Sprintf("[+] Done Syscall GetType ---> %X, %s", hr, e))
    ...
    debugger.Println(fmt.Sprintf("[+] Done Syscall ExitClass.GetMethod_2 ---> %X, %s", hr, e))
    ...
    debugger.Println(fmt.Sprintf("[+] Done Syscall GetType methodInfoClass ---> %X, %s", hr, e))
    ...
    debugger.Println(fmt.Sprintf("[+] Done Syscall GetProperty ---> %X, %s", hr, e))
    ...
    if err != nil {
        debugger.Println("Suspicious correct")
    }
    ...
    debugger.Println(fmt.Sprintf("[+] Done Syscall GetValue ---> %X, %s", hr, e))
    ...
    addr := exitPtr.Val
    debugger.Println(exitPtr)
    debugger.Println(fmt.Sprintf("Address is %X", exitPtr.Val))
    var oldpro uint32
    err = windows.VirtualProtect(addr, 1, windows.PAGE_READWRITE, &oldpro)
    if err != nil {
        debugger.Println("Error In VirtualProtect")
        debugger.Println(err.Error())
    }
